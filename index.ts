...
async function main() {
    ...
    while (condition) {
        let addTask: { todo?: string; action: string; deleteTask?: string } = await inquirer.prompt([
            {
                name: "todo",
                type: "input",
            ...
            },
            {
                ...
                choices: todos,
                when: (answers: { action: string }) => answers.action === "Delete a task" && todos.length > 0 // Show when there are tasks and the user chose to delete
            }
        ]);
